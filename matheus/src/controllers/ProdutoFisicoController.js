...
const DAO = require('../services/ProdutoFisicoDAO');
...
exports.adicionar = async (request, response, next) => {
	try {
		...
		return (instancia ? response.status(status.CREATED).send({produto_fisico_id:instancia.id}) : response.status(status.BAD_REQUEST).send());
