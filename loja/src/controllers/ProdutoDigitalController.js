 ...
 * Loja-ESCOS12
 * Dev:  
 * Desc.: Controlador de todas as funções relacionadas à entidade "produto_digital"
 */

const status = require("http-status");
const DAO = require('../services/ProdutoDigitalDAO');
...
exports.adicionar = async (request, response, next) => {
	try {
		...
		return (instancia ? response.status(status.CREATED).send({produto_digital_id:instancia.id}) : response.status(status.BAD_REQUEST).send());
