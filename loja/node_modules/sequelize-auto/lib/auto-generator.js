...
class AutoGenerator {
        ...
        tableNames.forEach(table => {
            ...
            if (this.options.lang === 'ts') {
                ...
                needed.forEach(model => {
                    ...
                    const filename = types_1.recase(this.options.caseFile, model, this.options.singularize);
                    str += 'import type { ';
    ...
    addTypeScriptAssociationMixins(table) {
        const sp = this.space[1];
        const [mySchemaName, myTableName] = types_1.qNameSplit(table);
        const modelName = types_1.recase(this.options.caseModel, myTableName, this.options.singularize);
        let str = '';
        const needed = {};
            ...
            fkFieldNames.forEach(fkFieldName => {
                ...
                if (spec.isForeignKey) {
                    if ((!mySchemaName || spec.source_schema === mySchemaName) && spec.source_table === myTableName) {
                        const btModel = types_1.recase(this.options.caseModel, spec.foreignSources.target_table, this.options.singularize);
                        const btModelSingular = sequelize_1.Utils.singularize(btModel);
                        (_a = needed[btModel]) !== null && _a !== void 0 ? _a : (needed[btModel] = new Set());
                        ...
                        const hasModel = types_1.recase(this.options.caseModel, spec.foreignSources.source_table, this.options.singularize);
                        ...
                        (_b = needed[hasModel]) !== null && _b !== void 0 ? _b : (needed[hasModel] = new Set());
                        if (isOne) {
                            const hasModelSingular = sequelize_1.Utils.singularize(hasModel);
                            ...
                            const hasModelSingular = sequelize_1.Utils.singularize(hasModel);
                            const hasModelPlural = sequelize_1.Utils.pluralize(hasModel);
                    ...
                    if (spec.isPrimaryKey && (!mySchemaName || spec.foreignSources.target_schema === mySchemaName) &&
                        ...
                        if (otherKey) {
                            const otherModel = types_1.recase(this.options.caseModel, otherKey.foreignSources.target_table, this.options.singularize);
                            (_c = needed[otherModel]) !== null && _c !== void 0 ? _c : (needed[otherModel] = new Set());
                            const otherModelSingular = sequelize_1.Utils.singularize(otherModel);
                            const otherModelPlural = sequelize_1.Utils.pluralize(otherModel);
    ...
    getTypeScriptFieldType(fieldObj, attr) {
        ...
        if (this.isNumber(fieldType)) {
            ...
            let values = [];
            if (fieldObj.special)
                values = fieldObj.special.map((v) => `"${v}"`); // postgres
            else { // mysql
